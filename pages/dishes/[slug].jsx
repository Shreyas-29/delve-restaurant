...
import { getDishDetails, getDishes } from '../../services'
...
const Dish = ({ dish }) => {
    ...
    const { featuredImage, title, description, price, review } = dish;
...
export async function getStaticProps({ params }) {
    const data = await getDishDetails(params.slug);
    return {
        props: {
            dish: data,
        },
...
export async function getStaticPaths() {
    ...
    return {
        paths: dishes.map(({ node: { slug } }) => ({ params: { slug } })),
        fallback: true,
    };
}